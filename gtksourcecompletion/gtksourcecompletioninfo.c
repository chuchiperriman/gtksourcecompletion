...
G_DEFINE_TYPE(GscInfo, gsc_completion_info, GSC_TYPE_WINDOW);

#define GSC_COMPLETION_INFO_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), GSC_TYPE_SOURCE_COMPLETION_INFO, GscInfoPrivate))
                            ...
                            gint                    *vscroll)
{
	...
	*vscroll = 0;
	...
	{
		*border = gtk_container_get_border_width (GSC_CONTAINER (info));
		
		scrollbar = gtk_scrolled_window_get_hscrollbar (GSC_SCROLLED_WINDOW (info->priv->scroll));

		if (GSC_WIDGET_VISIBLE (scrollbar))
		{
			*hscroll = scrollbar->allocation.height;
		}

		scrollbar = gtk_scrolled_window_get_vscrollbar (GSC_SCROLLED_WINDOW (info->priv->scroll));
...
window_resize (GscInfo *info)
{
	...
	gint hscroll;
	gint vscroll;
	GtkStyle *style = GSC_WIDGET (info)->style;

	gtk_window_get_default_size (GSC_WINDOW (info), &width, &height);
	...
	if (info->priv->widget != NULL)
	{
		...
		off = (gtk_container_get_border_width (GSC_CONTAINER (info)) + border) * 2;
	...
	gtk_window_resize (GSC_WINDOW (info), width, height);
}
...
gsc_completion_info_init (GscInfo *info)
{
	...
	/* Tooltip style */
	gtk_window_set_title (GSC_WINDOW (info), _("Completion Info"));
	gtk_widget_set_name (GSC_WIDGET (info), "gtk-tooltip");
	gtk_widget_ensure_style (GSC_WIDGET (info));
	
	gtk_window_set_type_hint (GSC_WINDOW (info),
				  GDK_WINDOW_TYPE_HINT_NORMAL);

	gtk_window_set_default_size (GSC_WINDOW (info), 300, 200);
	gtk_container_set_border_width (GSC_CONTAINER (info), 1);
}
...
gsc_completion_info_show (GtkWidget *widget)
{
	...
	GSC_WIDGET_CLASS (gsc_completion_info_parent_class)->show (widget);	
}
...
{
	GSC_WIDGET_CLASS (gsc_completion_info_parent_class)->expose_event (widget, expose);
...
{
	return g_object_new (GSC_TYPE_SOURCE_COMPLETION_INFO, 
					 ...
					 GtkTextIter             *iter)
{
	...
	g_return_if_fail (GSC_IS_SOURCE_COMPLETION_INFO (info));
	g_return_if_fail (GSC_IS_SOURCE_VIEW (view));
	
	if (iter == NULL)
	{
		buffer = gtk_text_view_get_buffer (GSC_TEXT_VIEW (view));
	...
	gsc_completion_utils_move_to_iter (GSC_WINDOW (info),
						  GSC_SOURCE_VIEW (view),
						  &start);
}
...
create_scrolled_window (GscInfo *info)
{
	...
	gtk_scrolled_window_set_policy (GSC_SCROLLED_WINDOW (info->priv->scroll),
					GSC_POLICY_AUTOMATIC,
					GSC_POLICY_AUTOMATIC);

	gtk_scrolled_window_set_shadow_type (GSC_SCROLLED_WINDOW (info->priv->scroll),
	                                     GSC_SHADOW_NONE);
	gtk_widget_show (info->priv->scroll);
	gtk_container_add (GSC_CONTAINER (info), info->priv->scroll);
}
				       ...
				       GtkWidget               *widget)
{
	...
	g_return_if_fail (GSC_IS_SOURCE_COMPLETION_INFO (info));
	g_return_if_fail (widget == NULL || GSC_IS_WIDGET (widget));
	...
	if (info->priv->widget != NULL)
	{
		...
		gtk_container_remove (GSC_CONTAINER (gtk_widget_get_parent (info->priv->widget)),
		                      info->priv->widget);
	...
	if (widget != NULL)
	{
		...
		if (use_scrolled_window (info, widget))
		{
			...
			if (needs_viewport (widget))
			{
				...
				gtk_viewport_set_shadow_type (GSC_VIEWPORT (child), GSC_SHADOW_NONE);
				gtk_widget_show (child);

				gtk_container_add (GSC_CONTAINER (child), widget);
			}
			
			gtk_container_add (GSC_CONTAINER (info->priv->scroll), child);
		}
		else
		{
			gtk_container_add (GSC_CONTAINER (info), widget);
		}
...
{
	g_return_val_if_fail (GSC_IS_SOURCE_COMPLETION_INFO (info), NULL);
